...
def _parse_bool(value: object, field_name: str = "stream") -> bool | None:
    ...
    if text in {"false", "0", "no", "n", "off"}:
        return False
    raise HTTPException(status_code=400, detail={"error": f"{field_name} must be a boolean"})
...
def _payload_from_fields(fields: dict[str, Any]) -> dict[str, Any]:
    ...
    preserve_subject = _parse_bool(preserve_value, "preserve_subject")
    if preserve_subject is not None:
        payload["preserve_subject"] = preserve_subject
    ...
    if "turn_id" in fields:
        payload["turn_id"] = _clean(fields.get("turn_id"))
    if "product_id" in fields:
        payload["product_id"] = _clean(fields.get("product_id"))
    if "template_id" in fields:
        payload["template_id"] = _clean(fields.get("template_id"))
    if "batch_id" in fields:
        payload["batch_id"] = _clean(fields.get("batch_id"))
    if "batch_index" in fields:
        payload["batch_index"] = _clean(fields.get("batch_index"))
    if "batch_total" in fields:
        payload["batch_total"] = _clean(fields.get("batch_total"))
    if "reference_upload_ms" in fields:
        payload["reference_upload_ms"] = _clean(fields.get("reference_upload_ms"))
    if "reference_cache_hits" in fields:
        payload["reference_cache_hits"] = _clean(fields.get("reference_cache_hits"))
    return payload
...
def _download_image_url(url: str) -> ImageInput:
    ...
    try:
        response = requests.get(
            source,
            headers={"Accept": "image/*,*/*;q=0.8", "User-Agent": "lgwraw image fetcher"},
            timeout=60,
...
def collect_http_image_urls(sources: list[ImageSource]) -> list[str]:
    """Keep public image URLs for relay providers that require URL references."""
    urls: list[str] = []
    for source in sources:
