...
def _warp_runtime_defaults() -> dict[str, Any]:
    clearance_enabled = _env_bool("LGWRAW_PROXY_RUNTIME_CLEARANCE_ENABLED", True)
    clearance_mode = os.getenv("LGWRAW_PROXY_RUNTIME_CLEARANCE_MODE", "flaresolverr").strip().lower()
    ...
    egress_mode = os.getenv("LGWRAW_PROXY_RUNTIME_EGRESS_MODE", "single_proxy").strip().lower()
    ...
    runtime.update(
        {
            "enabled": _env_bool("LGWRAW_PROXY_RUNTIME_ENABLED", True),
            "egress_mode": egress_mode,
            "proxy_url": os.getenv("LGWRAW_PROXY_RUNTIME_PROXY_URL", "http://privoxy:8118").strip(),
            "resource_proxy_url": os.getenv("LGWRAW_PROXY_RUNTIME_RESOURCE_PROXY_URL", "").strip(),
            "skip_ssl_verify": _env_bool("LGWRAW_PROXY_RUNTIME_SKIP_SSL_VERIFY", False),
            "reset_session_status_codes": [
                int(part.strip())
                for part in os.getenv("LGWRAW_PROXY_RUNTIME_RESET_STATUS_CODES", "403").split(",")
    ...
    runtime["clearance"].update(
        {
            ...
            "mode": clearance_mode,
            "user_agent": os.getenv("LGWRAW_PROXY_RUNTIME_USER_AGENT", DEFAULT_USER_AGENT).strip() or DEFAULT_USER_AGENT,
            "browser": os.getenv("LGWRAW_PROXY_RUNTIME_BROWSER", "chrome").strip() or "chrome",
            "flaresolverr_url": os.getenv("LGWRAW_FLARESOLVERR_URL", "http://flaresolverr:8191").strip(),
            "timeout_sec": _env_int("LGWRAW_PROXY_RUNTIME_CLEARANCE_TIMEOUT_SEC", 60, 1),
            "refresh_interval": _env_int("LGWRAW_PROXY_RUNTIME_CLEARANCE_REFRESH_INTERVAL", 3600, 60),
            "warm_up_on_start": _env_bool("LGWRAW_PROXY_RUNTIME_WARM_UP_ON_START", False),
        }
    )
    return runtime
...
def main() -> int:
    config_path = Path(os.getenv("LGWRAW_CONFIG_FILE", "/app/config.json"))
    ...
    if _looks_like_repository_default(existing) or _env_bool("LGWRAW_PROXY_RUNTIME_FORCE", False):
