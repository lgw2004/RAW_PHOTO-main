...
def create_storage_backend(data_dir: Path) -> StorageBackend:
    ...
    环境变量：
    - STORAGE_BACKEND: json|postgres|git (默认 json)
    ...
    if backend_type == "json":
    ...
    elif backend_type in ("postgres", "postgresql", "database"):
        # PostgreSQL 数据库存储
        database_url = os.getenv("DATABASE_URL", "").strip()
        if not database_url:
            raise ValueError("DATABASE_URL is required when using PostgreSQL storage")
        print(f"[storage] Using PostgreSQL storage: {_mask_password(database_url)}")
        ...
        raise ValueError(
            ...
            f"Supported backends: json, postgres, git"
        )
