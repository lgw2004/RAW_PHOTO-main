...
本文介绍 LGWRAW 的常见部署方式，以及后续升级项目时需要保留的数据和执行步骤。
...
```bash
git clone git@github.com:basketikun/lgwraw.git
cd lgwraw
```
...
```yaml
environment:
  - LGWRAW_AUTH_KEY=your_secret_key
```
...
```text
LGWRAW_AUTH_KEY=your_secret_key_here
```
...
```bash
docker logs -f lgwraw-warp
docker logs -f lgwraw-flaresolverr
```
...
```bash
mkdir -p backups
tar -czf backups/lgwraw-$(date +%Y%m%d-%H%M%S).tgz config.json .env data
```

如果没有 `.env`，可以去掉：

```bash
tar -czf backups/lgwraw-$(date +%Y%m%d-%H%M%S).tgz config.json data
```
